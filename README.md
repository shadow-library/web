...
- **`ApiError`** / **`ErrorResponse`** / **`ErrorField`** — the single error surface every transport raises. Carries `status`, machine `code`, `type`, field-level problems, and `retryAfterSeconds`.
...
- **`APIRequest`** — a chainable, thenable HTTP client for browser-origin calls (SPA apps, or anything that hits the API directly rather than through a server function).
...
```ts
import { ApiError, call, APIRequest, useDeviceId } from '@shadow-library/web';
```
